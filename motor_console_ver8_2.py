"""
motor_console_ver8_2.py -- ver8 を土台に、ID34/ID18 の二機同時MIT保持を追加した実験コンソール
...
# ---- ver8-2: 二機同時保持（ID34=0x22 AK80-9 / ID18=0x12 AK10-9） ----
# 「dual」は原点を打った安全姿勢からの短い二機試験だけに使う。
DUAL_MOTORS = ((34, "AK80-9"), (18, "AK10-9"))
DUAL_KP = 1.0
DUAL_KD = 0.5
DUAL_SEC = 3.0
DUAL_POS_MAX = 0.20         # MIT原点から ±0.20 rad
DUAL_CUR_ABORT = 1.0
DUAL_SPD_ABORT_DEG_S = 100.0
DUAL_MOVE_ABORT_DEG = 15.0
...
class Console:
    ...
    def zero(self):
        # ver8-2 では、例外・q・Ctrl+Cのどれでも二機を同時に止める。
        self.dual_zero()
    ...
    # ============================================================
    def dual_zero(self):
        """ID34/ID18 の両方に零MIT指令を送る。"""
        for _ in range(3):
            for mid, model in DUAL_MOTORS:
                self.bus.send(f_mit(mid, 0, 0, 0, 0, 0, model))
            time.sleep(0.01)

    def dual_preview(self, pos22, pos12):
        """dual が各モーターへ送る量子化後のフレームを、送らずに表示する。"""
        positions = (pos22, pos12)
        if any(abs(p) > DUAL_POS_MAX for p in positions):
            print(f"  中止: 目標位置は原点から ±{DUAL_POS_MAX:g} rad までです。")
            return False
        print(f"  dual: 50 Hz / Kp={DUAL_KP:g} / Kd={DUAL_KD:g} / 送信順 0x822 → 0x812")
        for (mid, model), pos in zip(DUAL_MOTORS, positions):
            fr = f_mit(mid, DUAL_KP, DUAL_KD, pos, 0, 0, model)
            got = unpack_mit(fr.data, model)
            print(f"  ID={mid:2d} (0x{mid:02X}) {model:7s}  pos={pos:+.4f} rad"
                  f"  DATA={fr.data.hex(' ')}"
                  f"  → Kp={got['kp']:.3f} Kd={got['kd']:.3f} pos={got['pos']:+.4f}")
        return True

    def _dual_state(self, mid, start_pos):
        s = self.bus.state(mid)
        if s is None or not s.alive(MAX_AGE):
            raise Aborted(f"ID={mid} のフィードバックが {MAX_AGE:.1f} 秒以上途絶")
        if s.src != "servo":
            raise Aborted(f"ID={mid} の応答形式が [{s.src}]")
        if s.err:
            raise Aborted(f"ID={mid} err={s.err}（{ERR_NAMES.get(s.err, '?')}）")
        if abs(s.cur) > DUAL_CUR_ABORT:
            raise Aborted(f"ID={mid} |電流|={abs(s.cur):.2f} A > {DUAL_CUR_ABORT:.1f} A")
        spd_deg_s = s.spd / ERPM_PER_DEG_S
        if abs(spd_deg_s) > DUAL_SPD_ABORT_DEG_S:
            raise Aborted(f"ID={mid} 速度={spd_deg_s:.1f} deg/s > {DUAL_SPD_ABORT_DEG_S:.0f}")
        if abs(s.pos - start_pos[mid]) > DUAL_MOVE_ABORT_DEG:
            raise Aborted(f"ID={mid} 移動量={s.pos - start_pos[mid]:+.1f} deg > {DUAL_MOVE_ABORT_DEG:.0f}")
        return s, spd_deg_s

    def dual(self, pos22, pos12, seconds):
        """二台を同一50 Hz予定表で保持し、個別の状態を監視・CSV保存する。"""
        if not 0 < seconds <= DUAL_SEC:
            print(f"  中止: 秒数は 0 より大きく {DUAL_SEC:g} 秒以下です。")
            return
        if not self.dual_preview(pos22, pos12):
            return
        states = {}
        for mid, _model in DUAL_MOTORS:
            s = self.bus.state(mid)
            if s is None or not s.alive(MAX_AGE):
                print(f"  中止: ID={mid} のフィードバックが来ていません。送信しません。")
                return
            if s.err:
                print(f"  中止: ID={mid} err={s.err}（{ERR_NAMES.get(s.err, '?')}）。送信しません。")
                return
            if abs(s.spd) > 50:
                print(f"  中止: ID={mid} が静止していません（{s.spd:.0f} ERPM）。")
                return
            states[mid] = s
        start_pos = {mid: s.pos for mid, s in states.items()}
        rows, abort = [], None
        period = 1.0 / SEND_HZ
        t0, next_send, tick = time.time(), time.time(), 0
        max_skew_ms = 0.0
        print(f"  ▶ dual（{seconds:.1f} 秒、開始位置 ID34={start_pos[34]:+.1f} / ID18={start_pos[18]:+.1f} deg）")
        try:
            while True:
                now = time.time()
                elapsed = now - t0
                ...
                if now < next_send:
                    ...
                    continue
                sent = []
                for (mid, model), pos in zip(DUAL_MOTORS, (pos22, pos12)):
                    sent.append(time.time())
                    self.bus.send(f_mit(mid, DUAL_KP, DUAL_KD, pos, 0, 0, model))
                skew_ms = (sent[1] - sent[0]) * 1000.0
                max_skew_ms = max(max_skew_ms, skew_ms)
                for mid, _model in DUAL_MOTORS:
                    s, spd_deg_s = self._dual_state(mid, start_pos)
                    rows.append((elapsed, tick, skew_ms, mid, s.pos, spd_deg_s, abs(s.cur), s.temp, s.err, s.age()))
                tick += 1
            ...
            self.dual_zero()
            print("  零MIT指令を ID34 / ID18 の両方へ3回送りました。")
        os.makedirs(self.log_dir, exist_ok=True)
        fn = os.path.join(self.log_dir, "dual_" + time.strftime("%H%M%S") + ".csv")
        try:
            with open(fn, "w", newline="", encoding="utf-8") as f:
                w = csv.writer(f)
                w.writerow(("elapsed_s", "tick", "skew_ms", "id", "pos_deg", "speed_deg_s", "abs_current_a", "temp_c", "err", "age_s"))
                w.writerows(rows)
            print(f"  {'完了' if abort is None else '中止'}: {tick} 周期 / 最大送信skew {max_skew_ms:.3f} ms / CSV: {fn}")
        except Exception as e:
            print(f"  ⚠ dual CSV 保存失敗: {e}")
        ...
        while True:
            ...
            try:
                if c == "q":
                    ...
                    if len(p) != 3:
                        print("  使い方: dualpreview 目標22rad 目標12rad")
                    else:
                        self.dual_preview(float(p[1]), float(p[2]))
                elif c == "dual":
                    if len(p) not in (3, 4):
                        print("  使い方: dual 目標22rad 目標12rad 秒  （秒は省略時2、最大3）")
                    else:
                        self.dual(float(p[1]), float(p[2]), num(3, 2.0))
                elif c == "jit":
                    self.jit(num(1, 5.0))
                elif c == "t":
                    ...
                    self.dual_zero()
                    print("  ID34 / ID18 の両方へ零MIT指令を送りました（ソフト停止。非常停止は電源OFF）")
                else:
...
HELP = f"""
motor_console_ver8_2 -- ver8 の二機同時MIT保持版（電流は大きさだけで判定）。plan で今日の順番。
...
二機       dualpreview 0.1 0 / dual 0 0 2 / dual 0.1 0 2       ← ID34=22番/AK80、ID18=12番/AK10。原点をo 0した安全姿勢だけ
...
二機同時（ID34=上位機22番/AK80-9、ID18=上位機12番/AK10-9。二軸が干渉しない安全姿勢だけ）
  0. s → id 34 → model AK80-9 → s → id 18 → model AK10-9 → s   両方の feedback/err=0/静止を確認
  1. 安全姿勢を作る → id 34 → o 0 → id 18 → o 0                 この試験用の共通MIT原点。o 0は軸を動かさない
  2. dualpreview 0 0                                               送信せずID・機種・量子化フレームを確認
  3. dual 0 0 2                                                    二機ゼロ保持2秒。無中止ならCSVを確認
  4. dualpreview 目標22 目標12 → dual 目標22 目標12 2              正方向・干渉なしを確認済みの小目標だけ
  5. q                                                            二軸へ零MIT指令を送り切断
"""
    ...
    if DRY_RUN:
        def arg(name, default):
            try:
                ...
                return default
        pos22, pos12, seconds = arg("--pos22", 0.0), arg("--pos12", 0.0), arg("--seconds", 2.0)
        if not 0 < seconds <= DUAL_SEC:
            print(f"dry-run 中止: --seconds は 0 より大きく {DUAL_SEC:g} 秒以下です")
            return
        if any(abs(x) > DUAL_POS_MAX for x in (pos22, pos12)):
            print(f"dry-run 中止: --pos22 / --pos12 は ±{DUAL_POS_MAX:g} rad までです")
            return
        print(f"dry-run（CAN未接続）: dual {pos22:g} {pos12:g} {seconds:g}")
        for (mid, model), pos in zip(DUAL_MOTORS, (pos22, pos12)):
            fr = f_mit(mid, DUAL_KP, DUAL_KD, pos, 0, 0, model)
            print(f"  ID=0x{mid:02X} {model:7s} Kp={DUAL_KP:.3f} Kd={DUAL_KD:.3f} pos={pos:+.4f} rad"
                  f" frame={fr.data.hex(' ')}")
        return
    windows_timer_1ms()
    if SIM_MODE:
